...
class Instagram_api {
	...
	protected $api_urls = array(
		...
		'hash_recent' => 'https://api.instagram.com/v1/tags/%s/media/recent?max_id=%s&min_id=%s&access_token=%s',
		'tags_search' => 'https://api.instagram.com/v1/tags/search?q=%s&access_token=%s',
		'hash_search' => 'https://api.instagram.com/v1/tags/search?q=%s&access_token=%s',
	...
	function instagramLogin()
	{
		...
		$login_url = 'https://api.instagram.com/oauth/authorize/?client_id=' . $this->config['instagram_client_id'];
		$login_url .= '&redirect_uri=' . $this->config['instagram_callback_url'];
		$login_url .= '&response_type=code';
		$login_url .= '&scope=' . $this->config['instagram_scope'];
	...
	function __apiCall($url, $post_parameters = FALSE)
	{
		...
		curl_setopt($curl_session, CURLOPT_SSL_VERIFYPEER, $this->config['instagram_ssl_verify']);
	...
	function authorize($code)
	{
		...
		return $this->__apiCall($authorization_url, "client_id=" . $this->config['instagram_client_id'] . "&client_secret=" . $this->config['instagram_client_secret'] . "&grant_type=authorization_code&redirect_uri=" . $this->config['instagram_callback_url'] . "&code=" . $code);
	}
	...
	function getPopularMedia()
	{
		...
		$popular_media_request_url = 'https://api.instagram.com/v1/media/popular?client_id=' . $this->config['instagram_client_id'];
	...
	function hashRecent($tag, $max_id = null, $min_id = null, $access_token)
	{

		$hash_recent_request_url = sprintf($this->api_urls['tags_recent'], $tag, $max_id, $min_id, $access_token);

		return $this->__apiCall($hash_recent_request_url);
	}
	...
	function hashSearch($hash, $access_token)
	{

		$tags_hash_request_url = sprintf($this->api_urls['hash_search'], $hash, $access_token);

		return $this->__apiCall($tags_hash_request_url);
	}
